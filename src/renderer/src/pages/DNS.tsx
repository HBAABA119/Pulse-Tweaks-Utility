  ...
  return (
    <>
      <Modal open={modalOpen} onClose={() => setModalOpen(false)}>
        <div className="bg-void-card p-6 rounded-2xl border border-void-border text-void-text w-[90vw] max-w-md">
          <h2 className="text-lg font-semibold mb-4">Confirm DNS Change</h2>
          {selectedProvider && (
            <>
              <p className="mb-4">
                You are about to change your DNS servers to{" "}
                <span className="text-void-primary font-medium">{selectedProvider.name}</span>.
              </p>
              <div className="bg-void-border-secondary border border-void-border p-3 rounded-md mb-4">
              ...
              <div className="space-y-2">
                {currentDNS.map((dns, index) => (
                  <div key={index} className="flex items-center gap-2 text-sm">
                    ...
                    <span className="text-void-text-secondary">{dns.servers}</span>
                  </div>
                ))}
              </div>
            ) : (
              <div className="flex items-center gap-2 text-sm text-void-text-secondary">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-4">
            {dnsProviders.map((provider) => (
              <Card
                ...
                disabled={loading}
                className="bg-void-card border border-void-border p-4 rounded-2xl hover:border-void-primary transition text-left"
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    <h3 className="font-semibold">
                      ...
                      {provider.recommended && (
                        <span className="text-xs text-void-primary ml-2">Recommended</span>
                      )}
                    </h3>
                ...
                <p className="text-sm text-void-text-secondary">{provider.description}</p>
                <div className="flex flex-wrap gap-1 mt-3">
                  {provider.features.map((feature, index) => (
                    <span key={index} className="px-2 py-1 bg-void-border text-xs rounded-md">
                      {feature}
                    </span>
                  ))}
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full px-3 py-2 bg-void-border border border-void-border-secondary rounded-lg text-void-text focus:outline-hidden focus:border-void-primary"
                    />
                  </div>
                  <div>
