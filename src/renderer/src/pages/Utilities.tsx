    ...
    <RootDiv>
        <div className="flex gap-4 flex-col mb-10 mr-4">
          ...
          {filteredUtilities.length === 0 && (
            <p className="text-void-text-secondary flex text-center items-center justify-center gap-2">
            ...
            filteredUtilities.map((util) => {
              return (
                <Card className="p-4 flex items-center gap-4" key={util.name}>
                  ...
                  <div>
                    <h1>{util.name}</h1>
                    <p className="text-sm  text-void-text-secondary">{util.description}</p>
                  </div>
                  <div className="flex justify-end ml-auto">
                    {util.type === "toggle" &&
                      (loadingStates[util.name] ? (
                        <div className="w-6 h-6 border-2 border-void-border-secondary border-t-void-primary rounded-full animate-spin" />
                      ) : (
                        <Toggle
