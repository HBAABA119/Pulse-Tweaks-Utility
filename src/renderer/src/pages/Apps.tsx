      ...
      try {
        ...
        if (import.meta.env.DEV) {
          ...
          const response = await fetch(
            "https://raw.githubusercontent.com/parcoil/void/refs/heads/v2/src/renderer/src/assets/apps.json",
          )
    ...
    <>
      <Modal open={importModalOpen} onClose={() => setImportModalOpen(false)}>
        <div className="bg-void-card border border-void-border rounded-2xl p-6 shadow-xl max-w-lg w-full mx-4">
          <h3 className="text-xl font-semibold text-void-text mb-3">
            ...
            {importedApps.length > 0 ? (
              <ul className="space-y-2 max-h-60 overflow-y-auto custom-scrollbar">
                {importedApps.map((id) => {
                  ...
                  return (
                    <li key={id} className="flex items-center gap-2 text-void-text">
                      <Checkbox
              ...
              <p className="text-void-text-secondary italic">No apps found in file</p>
            )}
          </div>
      ...
      <Modal open={!!loading} onClose={() => {}}>
        <div className="bg-void-card border border-void-border rounded-2xl p-6 shadow-xl">
          <div className="flex items-center gap-4">
            <div className="relative">
              <div className="w-10 h-10 border-4 rounded-full animate-spin border-t-void-primary border-void-accent"></div>
            </div>
            <div>
              <h3 className="text-lg font-medium text-void-text">
                {loading === "install" ? "Installing" : "Uninstalling"} {currentApp || "Apps"}
                <p className="text-sm text-void-text-secondary  mt-1 mb-1">
                  {totalApps > 0 && ` (${currentIndex} of ${totalApps})`}
                </p>
              </h3>
              <p className="text-sm text-void-text-secondary">This may take a few moments</p>
            </div>
          </div>
        </div>
      </Modal>
      <RootDiv>
        ...
        {!wingetInstalled && (
          <Card className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 w-full mt-5 flex gap-4 items-center">
            ...
            <div className="flex-1">
              <h1 className="font-medium text-void-text">Winget Not Installed</h1>
        ...
        {source === "Chocolatey" && !chocolateyInstalled && (
          <Card className="bg-amber-500/10 border border-amber-500/20 rounded-xl p-4 w-full mt-5 flex gap-4 items-center">
            ...
            <div className="flex-1">
              <h1 className="font-medium text-void-text">Chocolatey Not Installed</h1>
              <p className="text-void-text-secondary">
        ...
        <div className="flex gap-3 mt-5 w-auto ml-1 mr-1">
          <Button
            className="text-void-text flex gap-2"
          ...
          <label className="flex gap-2 cursor-pointer bg-void-border text-void-text rounded-lg font-medium px-3 py-1.5 text-sm text-center items-center active:scale-90 hover:bg-void-secondary transition-all duration-200">
          ...
          {selectedApps.length > 0 && (
            <Button
              className="flex gap-2 ml-auto bg-void-border text-void-text"
              variant="secondary"
        ...
        <p className="mb-2 mt-2 text-void-text-muted font-medium">
          Looking to debloat windows? its located in {""}
          <a className="text-void-primary cursor-pointer" onClick={() => router("/tweaks")}>
            Tweaks
          </a>
        </p>
        ...
        <div className="flex flex-row gap-2 items-center">
          ...
          <div className="ml-auto flex gap-2 items-center">
            <p className="text-void-text-muted">Select Source:</p>
            <Dropdown
        ...
        <div className="space-y-10 mb-10">
          <Suspense
            fallback={<div className="text-center text-void-text-secondary">Loading...</div>}
          >
            {Object.entries(appsByCategory).map(([category, apps]) => (
              <div key={category} className="space-y-4">
                <h2 className="text-2xl text-void-primary font-bold capitalize">{category}</h2>
                      ...
                      <Card key={appId} onClick={() => toggleApp(appId)} className="p-4">
                        <div className="flex items-center justify-between">
                          <div className="flex items-center gap-4">
                            ...
                            <div className="min-w-10 max-w-10 max--h-10 min-h-10 rounded-lg overflow-hidden bg-void-accent flex items-center justify-center">
                              {app.icon ? (
                                <img
                                  src={app.icon}
                                  alt={app.name}
                            ...
                            <div>
                              <h3 className="text-void-text font-medium group-hover:text-void-primary transition">
                                {app.name}
                              </h3>
                              {app.info && (
                                <p className="text-sm text-void-text-secondary line-clamp-1 font-semibold">
                                  {app.info}
                                </p>
                              )}
                              <p className="text-xs text-void-text-secondary">ID: {appId}</p>
                            </div>
                          </div>
                          {app.link && (
                            <div onClick={(e) => e.stopPropagation()}>
                              <button
                                ...
                                className="ml-3 text-void-primary hover:text-void-text-secondary transition-opacity"
                                onClick={(e) => {
                                  e.stopPropagation()
                                  try {
          ...
          <p className="text-center text-void-text-muted">
            Request more apps or make a pull request on{" "}
            <a
              href="https://github.com/parcoil/void"
              target="_blank"
