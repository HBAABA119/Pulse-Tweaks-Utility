  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <button
        ...
        className="px-4 py-2 rounded-lg border border-void-border bg-void-bg text-void-text hover:border-void-primary transition-all duration-200 flex items-center gap-2 min-w-[180px] justify-between shadow-sm hover:shadow"
      >
        <span>{value}</span>
        <ChevronDown
        ...
        {options.map((option, index) => (
          <button
            ...
            className={`w-full px-4 py-1.5 text-left transition-all duration-200 relative group ${
              value === option
                ? "bg-void-primary text-white font-medium"
                : "text-void-text hover:bg-void-border"
            }`}
          >
